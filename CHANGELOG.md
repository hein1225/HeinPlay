...
<details open>
<summary><h2 style="display: inline;">1.3.4</h2></summary>
...
- **直播换台优化开关**
  - 直播设置新增「无缝换台」：开启后换台时当前画面继续播放，目标频道在后台预载就绪后再切换（最多等待数秒，超时直接切换）；预载期间同时解码两路直播流，会消耗更多流量与设备性能，默认关闭。
  - 直播设置新增「FCC 快速换台」：优先使用直播源提供的 FCC 地址拉流以缩短换台等待；源未提供 FCC 地址的频道自动回退普通地址，默认关闭。
  - **直播源 EXO 音频软解增强**：Android 直播路径接入 media3 ffmpeg 解码器（FFmpegAudioRenderer），可软解 MediaCodec 硬件解码不支持的音频编码（如 mp2 / AC3 / EAC3 / AAC 等），修复部分直播源因音频编码不支持而黑屏、卡顿或无声的问题；仅 Android 直播启用，点播与其它平台维持原硬解，不受影响。

#### 修复

- **rtsp 直播源支持**：新增对 rtsp 协议直播源的识别，修复此前 ExoPlayer 下 rtsp 源无法播放的问题。
- **直播源缓存未按设置时间自动更新**：缓存写入不再重置过期时间，到期才回源刷新，确保按设置周期更新直播源。
- **首页「继续播放」与「我的」播放记录重复同步**：全平台两者现已共用同一份本地播放记录，避免重复同步与数据不一致。
- **直播回放拖动 / 长按快进快退**：定位节流，长时间按住不再卡死，定位更跟手。
- **直播网络源加载失败自动重试**：进入频道初始化失败时自动重试最多 3 次（间隔约 400ms），缓解源站负载均衡调度到异常节点导致的播放失败。
- **直播默认播放器后端调整**：Android / TV 与 Windows 直播默认改回 ExoPlayer（兼容性最佳），Linux / HarmonyOS 仍使用 fvp；个别设备连接 IPTV 源超时可在「直播设置 → 直播默认播放器」切回 fvp。
