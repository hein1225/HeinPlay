...
class NativePlayer extends PlatformPlayer {
  ...
  Future<void> _create() {
    return lock.synchronized(() async {
      ...
      // ao = null
      //
      // 精简版 mpv（flutter_mpv_libs_android_video）不支持 seek-to-file-pos、
      // demuxer-max-back-duration、cache-initial/min/back/default、seekable、
      // index、avindex、index-mode 等属性，移除以避免大量 property not found 日志。
      final properties = <String, String>{
        ...
        'hdr-compute-peak': 'no',
        'demuxer-max-bytes':
            (configuration.bufferSize * 3).toString(), // 3x buffer
        'demuxer-max-back-bytes':
            (configuration.bufferSize * 2).toString(), // 2x back
        'subs-fallback': 'yes',
        'subs-with-matching-audio': 'yes',
        // Android 精简版 mpv 不支持以下属性，桌面端（尤其是 Windows）需要它们才能
        // 正确识别 HLS/本地代理流的可 seek 性与缓存，避免一播放就显示结束或只缓存 20 秒。
        if (!Platform.isAndroid) ...{
          'cache-on-disk': 'yes',
          'hr-seek': 'yes',
          ...
          'demuxer-max-back-duration': '1200',
          'cache-secs': '600',
          'cache-initial': '4M',
          'cache-min': '16M',
          'cache-back': '200M',
          'cache-default': '${configuration.bufferSize ~/ 1024 / 1024}M',
          // seekable 在新版 mpv 中可能为只读，使用 force-seekable 强制可 seek
          'force-seekable': 'yes',
          'index': 'yes',
          'avindex': 'yes',
          'index-mode': 'both',
      ...
      if (configuration.videoPerformance?.optimizeForLocalFiles != false) {
        properties.addAll({
          ...
          if (Platform.isAndroid) ...{
            'cache-secs': '120', // Cache 2 minutes ahead
            'hr-seek': 'yes', // HR-seek for smooth playback
            'hr-seek-framedrop': 'yes', // Drop frames during seek only
          },
        });
      ...
      properties.addAll(
        {
          // 精简版 mpv 可能不支持 osc/osd-level，直接跳过避免 property not found 日志。
      ...
      if (configuration.videoPerformance != null) {
        ...
        properties.addAll({
          ...
          if (video.interpolation) 'interpolation': 'yes',
